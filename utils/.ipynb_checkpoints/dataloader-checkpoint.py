...
def config1(data):
    if data == 'shark':
        train_root = 'shark/train'
        test_root = 'shark/test'
        train_pd = pd.read_csv("shark/shark_train.txt", sep=" ", header=None,
                               names=['ImageName', 'label'])
        test_pd = pd.read_csv("shark/shark_test.txt", sep=" ", header=None,
                              names=['ImageName', 'label'])
        cls_num = 14
